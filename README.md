...
시그널링(`p2p.js`)은 인증 없이 `p2p` 경로에 offer/answer/ICE 후보를 쓰므로, 해당 경로만 읽기/쓰기를 허용하고 나머지는 차단합니다(저장소의 `database.rules.json` 와 동일):
...
> ⚠️ `p2p` 경로만 인증 없이 공개 읽기/쓰기이며 시그널링 메타데이터(offer/answer/ICE)만 오갑니다. 실제 게임 데이터는 P2P 데이터채널로만 흐릅니다. `p2p` 외 모든 경로는 기본 차단됩니다.
>
> 💡 테스트 모드로 만들었다면 30일 후 만료되므로, 만료 전 위 규칙으로 교체해 두어야 대전이 계속됩니다.
